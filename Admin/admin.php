<?php
// يجب أن يكون هذا السطر الأول في الملف بدون أي مسافات أو أسطر قبله
...
$items_query = "SELECT 
    ...
    oi.price,
    oi.product_image,
    CONVERT(oi.product_name USING utf8mb4) AS product_name,
    CONVERT(IFNULL(oi.product_name, p.name) USING utf8mb4) AS final_product_name,
...
if (isset($_POST['send_invoice'])) {
    ...
    foreach ($items as $item) {
        $product_name = $item['product_name'] ?? $item['final_product_name'];
...
<head>
    ...
    <title>فاتورة الطلب #<?= htmlspecialchars($order_id, ENT_QUOTES | ENT_HTML5, 'UTF-8') ?></title>
